...
export default function ComparisonCard({ college1, college2 }: Readonly<ComparisonCardProps>) {
    ...
    const initData = () => {
        const items: any = [college1, college2].map((college) => {
            ...
            return {
                ...
                userScore: {
                    score: (getScore(college, 'success') + getScore(college, 'outcomes') + getScore(college, npt43Key) + getScore(college, 'economic_inclusion_score')) / 4
                },
        ...
        <div className={classes.comparisonCardContainer}>
            {
                ...
                <>
                    <div className={classes.comparisonCardCol}>
                        <div className={`${classes.comparisonRow} ${classes.comparisonRowHeader}`}>
                            {item1.name}
                        </div>
                        {
                            categories.map((category, index) => (
                                <div className={`${classes.comparisonRow}`} key={`comparisonRowCategoryItem1${category.key}${index}`}>
                                    {toLetterGrade(item1[category.key].score)}
                                    {
                                        item1[category.key].score > item2[category.key].score &&
                                        <CheckCircleIcon className={classes.comparisonCheckIconLeft} color="success" fontSize="small" />
                                    }
                                </div>
                            ))
                        }
                    ...
                    <div className={`${classes.comparisonLabelCol}`}>
                        ...
                        {
                            categories.map((category, index) => (
                                <div className={`${classes.comparisonRow} ${classes.comparisonRowLabel}`} key={`comparisonRowCategoryLabel${category.key}${index}`}>
                                    {category.label}
                                </div>
                            ))
                        }
                    ...
                    <div className={classes.comparisonCardCol}>
                        ...
                        {
                            categories.map((category, index) => (
                                <div className={`${classes.comparisonRow}`} key={`comparisonRowCategoryItem2${category.key}${index}`}>
                                    {toLetterGrade(item2[category.key].score)}
                                    {
                                        item1[category.key].score < item2[category.key].score &&
                                        <CheckCircleIcon className={classes.comparisonCheckIconRight} color="success" fontSize="small" />
                                    }
                                </div>
                            ))
                        }
