...
export default function selectedschool({school, removeSchool}: ISelectedCollege) {
  ...
  background: '#FFF' }}>
    <Grid container spacing={2}>
      <Grid item xs={8}>
        <Typography sx={{position:'relative',right:'-10%',top:'30%'}}>{school}</Typography>
      </Grid>
      <Grid onClick={() => {removeSchool(school)}} item xs={4} sx={{position:'relative',right:'-20%',bottom:'-10%'}}>
