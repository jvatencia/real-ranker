...
def mergeCostColumns(df: pd.DataFrame):
    costPrefixes = ['NPT41', 'NPT42', 'NPT43', 'NPT44', 'NPT45']
    denanify = lambda x: 0 if np.isnan(x) else x
    for costPrefix in costPrefixes:
        costPrefix = costPrefix.lower()
        costPrefixCol = list()
        for i, row in df.iterrows():
            if ('Hebrew College' == row['instnm']):
                row1 = row['%s_pub' % costPrefix]
                row2 = row['%s_priv' % costPrefix]
                print(row1, type(row1), np.isnan(row1), row2, type(row2), np.isnan(row2))
            if (type(row) == type(None)):
                costPrefixCol.append(np.nan)
            else:
                row1 = row['%s_pub' % costPrefix]
                row2 = row['%s_priv' % costPrefix]
                # if (np.isnan(row1) and np.isnan(row2)):
                #     if ('Hebrew College' == row['instnm']):
                #         print('happened')
                #     costPrefixCol.append(np.nan)
                # else:
                costPrefixCol.append(denanify(row1) + denanify(row2))
        df[costPrefix] = costPrefixCol
    return df

def parse_new(src: str):
    ## 1) what's wrong with success?
    ## 2) maybe we take the mean not of the whole dataframe, but of every college in its group (defined by ccbasic, ccsizset, control)
    # load source csv
    ...
    for i, row in df.iterrows():
        ...
        support_eco_col.append(support.compute_eco_on_row(extraPercentile, extraPercentiles)) #DONE (i think)
        support_race_col.append(support.compute_race_on_row(row, extraPercentile, extraPercentiles, df)) #DONE (i think)
        debt_col.append(debt.compute_on_row(extraPercentile, extraPercentiles)) #DONE (i think)
        income_col.append(income.compute_on_row(extraPercentile, extraPercentiles))
        ppld_col.append(ppld.compute_on_row(extraPercentile, extraPercentiles))
    ...
    for i, row in df.iterrows():
        try:
            ...
            extraPercentile = None
        support_col.append(support.compute_on_row(extraPercentile, extraPercentiles)) #DONE (i think)
    ...
    for i, row in df.iterrows():
        try:
            ...
            extraPercentile = None
        success_col.append(success.compute_on_row(extraPercentile, extraPercentiles))
    ...
    df['success_absolute'] = df['success']
    df['success_relative'] = df['success']
    ...
    cost_cols = ['NPT41', 'NPT42', 'NPT43', 'NPT44', 'NPT45']
    for cost_col in cost_cols:
        added=getRelativeAndAbsolutePercentiles(df, cost_col.lower(), ascending=True)
    ...
    value_cols = ['value_1', 'value_2', 'value_3', 'value_4', 'value_5']
    from .utilities import getBalancedScoreDF
    for i, valueCol in enumerate(value_cols):
        cost = 'npt4%i' % (i+1)
        df[valueCol] =  getBalancedScoreDF('weighted_income') / getBalancedScoreDF(cost)
        added = getRelativeAndAbsolutePercentiles(df, valueCol, ascending=False)
        added['instnm'] = added['school']
        df = pd.merge(df, added, on='instnm', how='left')
        print(added.columns)




    df[colsWeWant+['st_fips', 'region']+scoreCols+spartans + value_cols].to_csv('drink.csv', index=False)
    # df.to_csv('drink2.csv', index=False)
from orderColumns import parseByLine, s2
print(parseByLine(s2))
def export():
    columnOrder = parseByLine(s2) + ['value_1_absolute','value_1_relative', 'value_2_absolute', 'value_2_relative', 'value_3_absolute', 'value_3_relative', 'value_4_absolute', 'value_4_relative', 'value_5_absolute', 'value_5_relative']
